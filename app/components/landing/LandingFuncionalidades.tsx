      ...
      <section className="py-20 bg-[#f9fafb]" id="funcionalidades">
        <div className="container mx-auto px-2 sm:px-4 lg:px-8">
          <div className="text-center max-w-2xl mx-auto mb-10 sm:mb-16">
            <h2 className="text-2xl xs:text-3xl sm:text-4xl font-bold text-[#0f172a] leading-tight">
              Acabe com a desorganização e o retrabalho
            </h2>
            <p className="mt-3 sm:mt-4 text-base xs:text-lg text-[#475569]">
              Centralize projetos, clientes, equipe e atendimento em um só lugar.
            </p>
          </div>
          <Tabs defaultValue={categoriasTabs[0].key} className="w-full">
            <TabsList className="flex flex-wrap gap-1 mb-8 sm:mb-10 bg-transparent px-0 sm:px-0">
              {categoriasTabs.map((cat) => (
                <TabsTrigger
                  key={cat.key}
                  value={cat.key}
                  className="data-[state=active]:bg-[#70645C] data-[state=active]:text-white px-2 sm:px-4 py-1 sm:py-2 rounded-full text-xs xs:text-sm sm:text-sm font-medium whitespace-nowrap min-w-[80px] xs:min-w-[100px] sm:min-w-[120px]"
                >
                  {cat.title}
                </TabsTrigger>
              ))}
            ...
            {categoriasTabs.map((cat) => (
              <TabsContent key={cat.key} value={cat.key} className="mt-0">
                <div className="flex flex-col-reverse lg:grid lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12 items-center">
                  <div className="order-2 lg:order-1 w-full">
                    <h3 className="text-lg xs:text-xl sm:text-2xl font-bold text-[#0f172a] mb-2 sm:mb-4 mt-4 sm:mt-6 lg:mt-0">
                      {cat.title}
                    </h3>
                    <p className="text-[#475569] mb-4 sm:mb-6 text-sm xs:text-base sm:text-lg">
                      {cat.description}
                    </p>
                    <ul className="space-y-2 sm:space-y-4">
                      {cat.bullets.map((item, i) => (
                        <li key={i} className="flex items-start text-xs xs:text-sm sm:text-base">
                          <CheckCircle size={18} className="text-[#70645C] mr-2 mt-1 flex-shrink-0" />
                          <span className="text-[#475569]">{item}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                  <div className="order-1 lg:order-2 w-full flex justify-center items-center mb-4 sm:mb-6 lg:mb-0">
                    <img
                      src={cat.image}
                      alt={cat.title}
                      className="w-full max-w-[320px] xs:max-w-[360px] sm:max-w-[420px] h-auto object-cover rounded-xl shadow-lg border border-[#70645C]/10"
                      loading="lazy"
                    />
                  </div>
                </div>
        ...
        <div className="container mx-auto px-2 sm:px-4 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {categoriasCards.map((cat) => (
              <div
                key={cat.key}
                className="bg-[#f9fafb] p-4 xs:p-6 rounded-xl border border-[#70645C]/10 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 flex flex-col items-center text-center"
              >
                <img
                  src={cat.image}
                  alt={cat.title}
                  className="w-full max-w-[280px] xs:max-w-[320px] sm:max-w-[400px] h-40 xs:h-48 sm:h-56 object-cover rounded-lg mb-3 sm:mb-4 shadow-md"
                  loading="lazy"
                />
                <h3 className="text-base xs:text-lg font-semibold text-[#0f172a] mb-1 sm:mb-2 group-hover:text-[#70645C] transition-colors">
                  {cat.title}
                </h3>
                <p className="text-[#475569] text-xs xs:text-sm mb-2 sm:mb-3">{cat.description}</p>
                <ul className="space-y-1 sm:space-y-2">
                  {cat.bullets.map((item, i) => (
                    <li key={i} className="flex items-start justify-center text-xs xs:text-sm">
                      <CheckCircle size={16} className="text-[#70645C] mr-2 mt-0.5 flex-shrink-0" />
                      <span className="text-[#475569]">{item}</span>
                    </li>
                  ))}
                </ul>
